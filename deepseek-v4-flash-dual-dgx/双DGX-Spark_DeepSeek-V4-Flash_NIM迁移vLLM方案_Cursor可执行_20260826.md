...
```bash
curl -fsS http://172.19.50.70:8000/v1/models \
...
```bash
curl -fsS http://172.19.50.70:18090/v1/models
```

Chat：

```bash
curl -sS http://172.19.50.70:18090/v1/chat/completions \
...
```bash
curl -f http://172.19.50.70:8000/v1/models
bash /opt/deepseek-v4/ops/scripts/status.sh
```
...
当前：
Node0 172.19.50.70 / CX7 192.168.100.10
