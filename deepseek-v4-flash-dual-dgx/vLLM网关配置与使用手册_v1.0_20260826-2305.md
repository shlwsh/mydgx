...
客户端 (opencode / Cursor / Open WebUI / 脚本)
        │
        ├── HTTP   http://172.19.50.70:4000/v1    ← LiteLLM 网关 HTTP（opencode 等，推荐）
        │             key 认证 + 模型别名
        ...
        │                                      vLLM :18090（推理后端）
        │
        └── HTTP  http://172.19.50.70:18090/v1   ← vLLM 直连（内网调试用）
                                                  │
                                                  ▼
...
| 服务 | 地址 | 认证 | 用途 |
|---|---|---|---|
| LiteLLM 网关（HTTP） | `http://172.19.50.70:4000/v1` | `sk-dgx-local-2026` | 客户端入口（推荐，无证书问题） |
| LiteLLM 网关（HTTPS） | `https://172.19.50.70:4443/v1` | `sk-dgx-local-2026` | 需加密场景（自签名证书） |
| vLLM 推理 | `http://172.19.50.70:18090/v1` | 无（内网） | 推理后端（内部/调试） |
| NIM（回滚） | `http://172.19.50.70:8000/v1` | 无 | 已停，保留回滚 |
...
openssl req -x509 -newkey rsa:2048 -nodes \
  ...
  -subj "/CN=172.19.50.70/O=DGX-Local" \
  -addext "subjectAltName=IP:172.19.50.70,IP:127.0.0.1,DNS:localhost" \
...
vLLM 暴露 OpenAI 兼容 API（无认证），`http://172.19.50.70:18090/v1`：

### 4.1 列模型
```bash
curl http://172.19.50.70:18090/v1/models
```
...
```bash
curl http://172.19.50.70:18090/v1/chat/completions \
...
```bash
curl -k https://172.19.50.70:4443/v1/chat/completions \
  -H 'Content-Type: application/json' \
  -H 'Authorization: Bearer sk-dgx-local-2026' \
  -d '{"model":"deepseek-coding","messages":[{"role":"user","content":"你好"}],"max_tokens":256}'
```
...
client = OpenAI(base_url="http://172.19.50.70:4000/v1",
...
```python
from openai import OpenAI
client = OpenAI(base_url="http://172.19.50.70:18090/v1", api_key="none")
resp = client.chat.completions.create(
